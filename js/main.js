...
import {drawDrops, drawPtc, spawnPtc, spawnDrop} from '../entities/particles.js';
import {playHit} from '../lib/audio.js';
...
resize();
const cam = { x: 0, y: 0, shake: 0, shake_x: 0, shake_y: 0 };
...
vsl.x += nx * overlap; vsl.y += ny * overlap;
const rel_v = vsl.vx * nx + vsl.vy * ny;
if(rel_v < 0){
vsl.vx -= 1.6 * rel_v * nx; vsl.vy -= 1.6 * rel_v * ny;
if(v_spd > 120){
...
vsl.x += nx * overlap * (e.mass / total_mass); vsl.y += ny * overlap * (vsl.mass / total_mass);
e.x -= nx * overlap * (vsl.mass / total_mass); e.y -= ny * overlap * (vsl.mass / total_mass);
const dvx = vsl.vx - e.vx, dvy = vsl.vy - e.vy;
const rel_vn = dvx * nx + dvy * ny;
if(rel_vn < 0){
const impulse = -(1.6) * rel_vn / (1/vsl.mass + 1/e.mass);
vsl.vx += (impulse / vsl.mass) * nx; vsl.vy += (impulse / vsl.mass) * ny;
e.vx -= (impulse / e.mass) * nx; e.vy -= (impulse / e.mass) * ny;
const impact_force = Math.abs(impulse);
if(impact_force > 400){
e.hp -= impact_force * 0.025 * ramMult; e.flash = 0.25;
if(vsl.invuln <= 0){
vsl.hp -= impact_force * 0.015 * damageReduction; vsl.invuln = 0.4;
}
cam.shake = Math.min(35, impact_force * 0.02); playHit(impact_force);
const pcx = (vsl.x + e.x) / 2, pcy = (vsl.y + e.y) / 2;
const p_count = Math.min(35, Math.floor(impact_force * 0.025));
for(let j = 0; j < p_count; j++){
const a = Math.random() * Math.PI * 2, s = Math.random() * impact_force * 0.25;
spawnPtc(pcx, pcy, Math.cos(a)*s, Math.sin(a)*s, 1.2, e.m_type === 'jelly' ? '#ff00ff' : '#ff2222', 2 + Math.random() * 5);
}
if(e.hp <= 0){
spawnDrop(e.x, e.y, Math.floor(e.max_hp * 0.4));
for(let j = 0; j < 45; j++){
const a = Math.random() * Math.PI * 2, s = Math.random() * 350;
...
vsl.hp -= 8 * damageReduction; vsl.invuln = 0.5; cam.shake = 8;
 }
  }
}
 }
...
cx.moveTo(x, cam.y * 0.3 - H/2); cx.lineTo(x, cam.y * 0.3 + H/2); 
}
for(let y = start_y; y < cam.y * 0.3 + H/2; y += grid_sz){
cx.moveTo(cam.x * 0.3 - W/2, y); cx.lineTo(cam.x * 0.3 + W/2, y); 
}
cx.stroke();
cx.restore();
}
...
const view_l = cam.x - W/2 - 150, view_r = cam.x + W/2 + 150;
const view_t = cam.y - H/2 - 150, view_b = cam.y + H/2 + 150;
for(let i = 0; i < activeEnts.length; i++){
const e = activeEnts[i];
...
const s = e.segs[j], ox = Math.sin(t + e.phase + j * 0.6) * (j * 2.5);if(j === 0) cx.moveTo(s.x + ox, s.y); else cx.lineTo(s.x + ox, s.y); 
 }
cx.stroke();
}
if(e.t === 'rock'){
...
const a = e.ang + (j / e.verts.length) * Math.PI * 2, r = e.r * e.verts[j], px = e.x + Math.cos(a) * r, py = e.y + Math.sin(a) * r;
if(j === 0)
cx.moveTo(px, py);
else 
cx.lineTo(px, py); 
...
grad.addColorStop(0, `rgba(255, 120, 20, ${0.4 + Math.sin(t + e.phase) * 0.15})`); grad.addColorStop(1, 'rgba(255, 50, 0, 0)');
...
if(!last_t)
last_t = t;
let frame_dt = (t - last_t) / 1000; last_t = t;
if(frame_dt > 0.1)
frame_dt = 0.1;
acc += frame_dt;
while(acc >= fixed_dt){
...
cam.x += (vsl.x - cam.x) * 0.12; cam.y += (vsl.y - cam.y) * 0.12;
if(cam.shake > 0){
cam.shake_x = (Math.random() - 0.5) * cam.shake; cam.shake_y = (Math.random() - 0.5) * cam.shake;
cam.shake *= 0.88; if(cam.shake < 0.5) cam.shake = 0;
} 
else{
cam.shake_x = 0; cam.shake_y = 0; 
}
drawBg();
drawWorld();
}
